...
class TheKingsArmyDiv2 {
    ...
    int getNumber(vector<string> state) {
    	int happy_cnt = 0, happy_pairs = 0;
	int r = state.size();
	int c = state[0].size();
	for(int i=0;i<state.size();i++){
		for(int j=0;j<state[i].length();j++){
			if(i < r-1)
				if(state[i][j] == 'H' && state[i+1][j] == 'H') happy_pairs++;
			if(j < c-1)
				if(state[i][j] == 'H' && state[i][j+1] == 'H') happy_pairs++;
			if(state[i][j] == 'H') happy_cnt++;
		}
	}
	if(happy_cnt == 0) return 2;
	if(happy_pairs > 0) return 0;
	return 1;
    }
};
