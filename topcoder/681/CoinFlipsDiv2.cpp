...
class CoinFlipsDiv2 {
    ...
    int countCoins(string state) {
        ...
        if(state.length() == 1) return 0;
            ...
            if(state[0] != state[1]) return 2;
        }
        for(int i=1; i<state.length()-1; i++) {
            if((state[i-1] != state[i]) || (state[i] != state[i+1])) result = result + 1;
        }
        if(state[0] != state[1]) result = result + 1;
        if(state[state.length() - 1] != state[state.length() - 2]) result = result + 1;
        return result;
    }
};
