...
class TaroFillingAStringDiv2 {
    public:
    bool get_cost(int len, bool same) {
        if(((len % 2 == 0) && !same) || ((len % 2 != 0) && same)) return 0;
        else return 1;
    }
    ...
    int getNumber(string s) {
        ...
        for(int i=0; i<s.length(); i++) {
            if(s[i] != '?') known.push_back(i);
        }
        int cost = 0;
        for(int i=0; i+1<known.size(); i++) {
            bool same = (s[known[i]] == s[known[i+1]]);
            cost = cost + get_cost(known[i+1] - known[i]-1, same);
        }
        return cost;
    }
};
